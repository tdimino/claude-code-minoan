...
## Sister Skill: `/minoan` (Lisan al-Qads)

This skill is the *analytical toolkit*. The `/minoan` skill (`~/.claude/skills/minoan/`) is the *language faculty*—it composes text in reconstructed Minoan, generates prayers/inscriptions/greetings, and provides pronunciation. When you need to *speak* Minoan rather than *analyze* it, invoke `/minoan`. When `/minoan` needs cognate verification, it calls scripts from this skill.
...
| SigLA signs | `assets/scripts/linear-a/sigla-sign-map.json` | 334 Linear A signs (AB numbers, Unicode, images) |
| Evolution map | `assets/scripts/sign-evolution-map.json` | 20 signs: CH→LA→LB lineage, PS cognates |
...
- `data/sign-registry.json` — Unified sign metadata (built by `build_registry.py`)
...
# Search all polyphonic readings (e.g., KI-RO tries both k-r and k-r skeletons)
uv run ~/.claude/skills/linear-a-decipherment/scripts/cognate_search.py "KI-RO" --all-readings
```
...
Look up signs from the unified registry (334 signs, polyphonic readings, CH→LA→LB evolution):
...
uv run ~/.claude/skills/linear-a-decipherment/scripts/sign_lookup.py AB67 --reading ku
```
...
├── SKILL.md                    # This file
├── assets/scripts/             # Bundled script assets (fonts, sign images, SVGs, JSONs)
│   ├── fonts/                  # NotoSansLinearA/B, CretanHieroglyphs, CyproMinoan, Vinča
│   ├── linear-a/signs/         # SigLA sign images (AB01_da.png, ...)
│   ├── proto-sinaitic/         # Proto-Sinaitic SVGs
│   ├── sign-evolution-map.json # 20-sign cross-script evolution map
│   └── linear-a/sigla-sign-map.json  # 334 SigLA sign metadata
├── lib/                        # Shared Python library
│   ├── __init__.py
│   ├── types.py                # Frozen dataclasses (Inscription, LexiconEntry, CognateMatch, SignReading, SignEntry)
│   ├── js_parser.py            # JS Map → Python dict extraction
│   ├── normalization.py        # normalize(), lookup_in(), J/Y swap
│   ├── skeleton.py             # SIGN_DECOMPOSITION, extract_skeleton(), load_registry(), extract_skeleton_polyphonic()
...
│   ├── sign_lookup.py          # CLI sign lookup (AB number, phonetic, unicode, polyphonic, evolution)
...
│   ├── knossos-scepter-2024.md # Knossos scepter analysis
│   └── ml-approaches.md        # Computational decipherment survey (v2)
└── data/                       # Pre-extracted, ships with skill
    ...
    └── sign-registry.json      # 334 signs: readings, rendering, evolution, Gordon refs
```
