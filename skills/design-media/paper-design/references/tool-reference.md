...
**`get_jsx` on shader nodes returns a `@paper-design/shaders-react` component.** When an artboard contains a Paper Shader element (Mesh Gradient, Fluted Glass, Liquid Metal, Halftone CMYK, etc.), `get_jsx` exports the matching React component with current prop values — not a raw HTML element. The consuming project must have `@paper-design/shaders-react` installed (`npm i @paper-design/shaders-react`) for the output to run. Shader prop names drift between Paper releases (e.g. Fluted Glass renamed `Edges` to `Stretch` in Nov 2025); when legacy exports misbehave, re-check the live prop table at `shaders.paper.design/<slug>`. See `shaders.md` for the full catalog.
