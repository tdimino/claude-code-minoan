# Linear — DESIGN.md Exemplar

Extracted design system in DESIGN.md format (the convention introduced by Google Stitch). Use as a reference for what a well-crafted DESIGN.md contains. When a project has its own DESIGN.md, read it alongside this to calibrate extraction depth.

Complements `stripe-design-tokens.md` (light-mode premium) and `mintlify-design-tokens.md` (dark-first docs). Linear represents the monochrome-speed aesthetic: keyboard-first SaaS, aggressive density, non-standard font weights.

Source: [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md/tree/main/design-md/linear.app/)
