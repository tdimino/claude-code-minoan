...
**Landing page components**: `marquee-component.md` (infinite logo ticker with animation, a11y, responsive speed), `gallery-component.md` (horizontal scroll showcase with mixed sizes, snap, lightbox), `research-card-component.md` (article cards with metadata, inline data viz, category tags), `article-layout.md` (long-form with sticky TOC sidebar, block quotes, citation system), `inline-dataviz.md` (CSS-only bar charts, stat callouts, animated counters, sparklines, comparison rows).
