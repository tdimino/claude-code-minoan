#!/usr/bin/env python3
"""Clone a voice with Qwen3-TTS Base model."""
import argparse
import time
import sys
import os
...
def main():
    parser = argparse.ArgumentParser(description="Clone voice with Qwen3-TTS")
    parser.add_argument("text", help="Text to synthesize")
    parser.add_argument("ref_audio", help="Reference audio file (3+ seconds)")
    parser.add_argument("-r", "--ref-text", default="", help="Transcript of reference")
    parser.add_argument("-o", "--output", default="cloned.wav", help="Output file")
    ...
    model_name = "Qwen/Qwen3-TTS-12Hz-0.6B-Base" if args.small else "Qwen/Qwen3-TTS-12Hz-1.7B-Base"
    ...
    print(f"Cloning from: {args.ref_audio}", file=sys.stderr)
    t_gen_start = time.time()
    wavs, sr = model.generate_voice_clone(
        text=args.text, language=args.lang, ref_audio=args.ref_audio,
