...
| `minoan-frontend-design` | Frontend design with eval infrastructure (70% win rate). 27 reference files: creative techniques, color science (meodai/color-expert), Stripe/Mintlify/Vercel design tokens, Nielsen heuristics scoring, Impeccable domain refs, Astryx hero pattern (depth-parallax + fluid-dom sibling links) |
...
| `threejs-particle-canvas` | Interactive Three.js particle canvases + WebGPU spinner/loaders. Two modes: narrative canvases (WebGL, ambient art) and parametric curve spinners (WebGPU + TSL, 9 curve types). Design references: Instance, Vellum |
...
| `sprite-forge` | Game sprites, SVG characters, ASCII art (static + animated), animated mascots, isometric turnarounds — 5 output modes with chongdashu pipeline, video-to-spritesheet, Gemini SVG-as-code, GSAP animation. 9 scripts, 11 references, 1 asset |
