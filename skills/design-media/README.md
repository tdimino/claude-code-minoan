...
| `scroll-cinema` | Cinematic scrolltelling: Lenis smooth scroll + GSAP ScrollTrigger cinematic reveals + Three.js painted-texture shader backgrounds reactive to cursor, scroll velocity, and chapter progress. 4 modes (`chapter-reveal`, `painted-backdrop`, `full-cinema`, `catalog`), 4 entrance patterns (`fade-up`, `split-text`, `scale-reveal`, `clip-path-wipe`), 3 shader presets, OKLCH color transitions, 13 gotchas, generator + 27-check validator |
