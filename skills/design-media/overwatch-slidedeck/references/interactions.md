# Overwatch Interaction Patterns
...
A deck reads as designed when every slide speaks the same motion language: the timing table above is the entire vocabulary. Choose entrance variants per slide, but never per-slide durations or easings — 20 slides with one grammar feel like a keynote (Google Cloud Next 2026 held one motion system across 3.5 hours of keynotes); 20 slides with bespoke timing feel like a template assembly. The `ease-out` entrances also carry meaning: fast-in-slow-settle reads as *response* — content arriving because the presenter advanced. Reserve `easeInOut` for autonomous motion (rotators, tickers) that moves on its own schedule.

Every animated component reads `useReducedMotion` (`hooks/useReducedMotion`) — a live `prefers-reduced-motion` subscription. Under reduced motion, entrances render their final state, rotators hold their first item, and the typewriter prints full text. New components follow the same contract: read the hook, skip the motion, keep the content.
