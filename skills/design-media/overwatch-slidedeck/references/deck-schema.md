...
A YAML spec serves as Claude's brief for composing each slide. Claude reads the spec alongside `slide-templates.md` to produce each slide `.tsx` file. This is not template codegen—it's structured input that Claude interprets with full creative latitude.
...
design:
  primaryColor: "#ff6e41"  # overrides --color-orange and --color-orange-muted
  fontHeading: "Geist"     # optional — overrides --font-heading, adds Google Fonts link
  fontBody: "Inter"        # optional — overrides --font-body, adds Google Fonts link
  transition: "fade"       # optional — deck-level default: "none" | "fade" | "slide" | "scale"
  transitionDuration: 400  # optional — deck-level default in ms

slides:
  - type: shader-cover
    id: "01-cover"
    mode: dark
    notes: "Remember to mention the fundraise" # optional — presenter notes
    transition: "slide"    # optional — per-slide override of deck transition
    transitionDuration: 600 # optional — per-slide override in ms
    # ... type-specific fields
```

### Common Per-Slide Fields

Every slide type supports these optional fields in addition to its type-specific fields:

| Field              | Type   | Default        | Description                                        |
|--------------------|--------|----------------|----------------------------------------------------|
| `notes`            | string | —              | Presenter notes shown in presenter view             |
| `transition`       | string | deck default   | Per-slide transition: `"none"`, `"fade"`, `"slide"`, `"scale"` |
| `transitionDuration` | number | deck default | Per-slide transition duration in milliseconds       |
...
2. Look up the template code in `slide-templates.md` by template number
...
1. Validates the spec (types, ids, required fields) — exits with all errors before any writes
2. Copies `scaffold/` to the target directory
3. Generates `config.ts` with the scaffold's full API (config, slides, slideModules, getSlideComponent, preloadSlide, slideList, totalSlides)
4. Creates empty slide files with correct naming (`{id}.tsx`)
5. Reports which components to import per slide type
6. Applies design overrides to `src/styles/globals.css` and `index.html` (primaryColor, fontHeading, fontBody)
