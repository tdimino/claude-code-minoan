# Overwatch Slide Templates

Fourteen slide type templates. Each shows the recommended component composition and mode (dark/white/orange).
...
## Layout Notes: Bento Grids and Metric Heroes

Two layout moves have displaced title-plus-bullets as the default for dense content, and both compose from existing components — no new template required:

**Metric hero.** One number owns the slide: a Headline-scale figure (`text-[140px]`, tabular-nums), an Eyebrow naming what it measures, one BodyText line of context. Everything else supports. Use when a single statistic *is* the argument — "1,324 documents. 40 seconds." — and resist adding a second panel to fill space; the emptiness is the emphasis.

**Bento grid.** Asymmetric modular cards in a `GridLayout`-derived arrangement: one 2×2 hero cell (headline metric or product visual), surrounded by 1×1 cells mixing a quote card (`SocialProofCard`), a small stat, an icon+label. The mix of content types per cell is the point — a bento of four identical stat cards is just a grid. Cap at 5-6 cells per slide; hover states (`HoverLift`) on cells keep the interactive character of the deck.

Data-slide rule that governs both: the headline states the insight, the visualization is its evidence. Annotate the datapoint that matters (orange accent, one callout) rather than decorating axes.
