...
export function DeckShell({ children, currentSlide, slides }: DeckShellProps) {
  ...
  useEffect(() => {
    const channel = new BroadcastChannel("overwatch-deck");
    const handler = (e: MessageEvent) => {
      const slideNum = Number(e.data?.slide);
      if (slideNum >= 1 && slideNum <= total && slideNum !== currentSlide) {
        navigate({ to: "/deck/$slide", params: { slide: String(slideNum) } });
      }
    };
