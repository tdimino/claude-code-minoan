...
| `fable` | Summon Claude Fable 5.1 (Theoros, xenos theoros) by default, or Fable 5.0 (Naos, `--naos`)—availability probe, two-lane invocation, boundary-safe model verification |
...
| `codex-cto` | [Codex CLI](https://github.com/openai/codex) as CTO: GPT-5.6-Sol plans, GPT-5.6-Sol reviews, Claude Code executes |
| `codex-orchestrator` | Orchestrate [OpenAI Codex CLI](https://github.com/openai/codex) subagents (GPT-5.6 Sol/Terra/Luna) for review, debug, architect, security |
...
| `keenable` | Web search + page fetch via [Keenable AI](https://keenable.ai) — point-in-time search, prompt-driven extraction, 100K free/mo |
