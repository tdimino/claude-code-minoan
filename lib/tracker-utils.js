...
function decodeProjectPath(dirName) {
  // Try reading projectPath from the most recent JSONL transcript
  ...
  try {
    ...
    if (files.length > 0) {
      const firstLine = fs.readFileSync(path.join(projDir, files[0].name), 'utf-8').split('\n')[0];
      const parsed = JSON.parse(firstLine);
      if (parsed.cwd) return parsed.cwd;
    }
  } catch (e) {
    // No transcripts or parse error — fall through
  }
