 ...
 * Singleton lazy-open pattern over node:sqlite (synchronous, built into Node,
 * no native module to rebuild when Node upgrades).
 *
 * The handle returned by getDb() keeps the better-sqlite3-shaped surface the
 * call sites were written against: prepare/exec/close plus pragma(str) and
 * transaction(fn). See wrap() below.
 */
...
if (NODE_MAJOR < 22) {
  throw new Error('tracker-db requires Node >= 22.13 (node:sqlite); found ' + process.versions.node);
}
...
let _db = null;
let _probe;          // undefined | true | false — cached isAvailable() result
let lastError = null;

/**
 * Give a DatabaseSync the shape every call site already expects.
 * - prepare(): statements ignore extra named keys (better-sqlite3 semantics)
 * - pragma(str): handles both "x = y" setters and "main.wal_checkpoint(TRUNCATE)"
 * - transaction(fn): BEGIN IMMEDIATE so busy_timeout is honored under WAL
 *   (a deferred read-then-write returns BUSY_SNAPSHOT without waiting);
 *   nested calls fall back to a SAVEPOINT
 */
// Named parameters (@name) used by a statement, parsed once per prepare().
// node:sqlite binds NULL for a key the caller forgot (better-sqlite3 threw),
// which would let a misspelled field silently write NULL rows; the wrapper
// below restores the throw. String literals in our SQL never contain '@'.
const NAMED_PARAM_RE = /@([A-Za-z_][A-Za-z0-9_]*)/g;
function namedParamsOf(sql) {
  const names = new Set();
  for (const m of sql.matchAll(NAMED_PARAM_RE)) names.add(m[1]);
  return [...names];
}
function isBindObject(v) {
...
function wrap(raw) {
  ...
  return {
    prepare(sql) {
      ...
      const names = namedParamsOf(sql);
      if (names.length) {
        const check = (args) => {
          if (args.length === 1 && isBindObject(args[0])) {
            for (const n of names) {
              if (!(n in args[0])) throw new TypeError('Missing named parameter "@' + n + '"');
            }
          }
        };
        for (const method of ['run', 'get', 'all']) {
          const orig = stmt[method].bind(stmt);
          stmt[method] = (...args) => { check(args); return orig(...args); };
        }
      }
      return stmt;
    },
    exec(sql) { return raw.exec(sql); },
    pragma(str) { return raw.prepare('PRAGMA ' + str).all(); },
    transaction(fn) {
      return (...args) => {
        if (raw.isTransaction) {
          const sp = 'tx_' + (++savepointSeq);
    ...
    get isTransaction() { return raw.isTransaction; },
...
function getDb() {
  ...
  _db = wrap(new DatabaseSync(DB_PATH));
...
/**
 * Real open-probe. A file that exists but cannot be opened (corrupt, locked
 * beyond the timeout, wrong engine) reports false and records why in
 * lastError, so callers can fall back loudly instead of crashing later.
 */
function isAvailable() {
  if (_probe !== undefined) return _probe;
  try {
    if (!fs.existsSync(DB_PATH) || fs.statSync(DB_PATH).size === 0) {
      lastError = new Error('tracker.db not found — run: node ~/.claude/scripts/migrate-to-sqlite.js');
      return (_probe = false);
    }
    const p = new DatabaseSync(DB_PATH, { readOnly: true, timeout: 1000 });
    try { p.prepare('SELECT 1 FROM sqlite_master LIMIT 1').get(); }
    finally { p.close(); }
    return (_probe = true);
  } catch (e) {
    lastError = e;
...
function initSchema() {
  ...
  // Summary provenance — every model- or cache-written summary carries its
  // source so it can be audited and erased without registry archaeology
  const sessionCols = db.prepare('PRAGMA table_info(sessions)').all().map(c => c.name);
  if (!sessionCols.includes('summary_source')) {
    db.exec('ALTER TABLE sessions ADD COLUMN summary_source TEXT');
  }
}
...
/**
 * Encoded project dir name → real project path, one row per dir, newest
 * session wins. project_path comes from the transcript's own cwd (written by
 * hooks/session-sync-db.py), so this is exact where the hyphen heuristic is
 * lossy ("Plasma AI", "Thera-Knossos-Minos-Paper"). Rows whose project_path
 * is still an encoded name (pre-repair) are skipped.
 */
function getProjectDirMap() {
  const rows = getDb().prepare(`
    SELECT project_dir, project_path FROM sessions
    WHERE project_path LIKE '/%'
    ORDER BY modified_at ASC`).all();
  const map = new Map();
  for (const r of rows) map.set(r.project_dir, r.project_path);   // later (newer) overwrites
  return map;
}

/**
 * session_id → { customTitle, summary } for every session in the DB. Replaces
 * the frozen session-registry.json as the metadata source for listings.
 */
function getSessionMetaMap() {
  const rows = getDb().prepare(
    'SELECT session_id, custom_title, auto_title, summary FROM sessions').all();
  const map = new Map();
  for (const r of rows) {
    map.set(r.session_id, { customTitle: r.custom_title || r.auto_title || '', summary: r.summary || '' });
  }
  return map;
}
